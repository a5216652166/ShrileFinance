namespace Application
{
    ...
    public class CreditContractAppService
    {
        ...
        public void Create(CreditContractViewModel model)
        {
            ...
            // 报文追踪(贷款合同信息记录)
            messageAppService.MessageTrack(id: credit.Id, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.签订授信合同, name: "签订授信合同：" + credit.CreditContractCode);

            // 报文追踪(担保合同信息记录)
            credit.GuarantyContract.ToList().ForEach(m =>
            {
                messageAppService.MessageTrack(id: m.Id, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.签订授信合同, name: "签订担保合同,担保人：" + m.Guarantor.Name);
            });
        ...
        public void Modify(CreditContractViewModel model)
        {
            ...
            if (creditGuarantyContractCount == 0 && model.GuarantyContract.Count > 0)
            {
                ...
                {
                    messageAppService.MessageTrack(id: m.Id.Value, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.合同变更, name: "授信合同：" + credit.CreditContractCode + "追加担保，担保人：" + m.Guarantor.Name);
                });

                // 报文追踪(主业务信息记录)
                messageAppService.MessageTrack(id: credit.Id, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.合同变更, name: "授信合同：" + credit.CreditContractCode + "变更");
            }
            else if (creditGuarantyContractCount > 0 && model.GuarantyContract.Count == 0)
            {
                ...
                model.GuarantyContract.ToList().ForEach(m =>
                {
                    messageAppService.MessageTrack(id: m.Id.Value, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.合同变更, name: "授信合同：" + credit.CreditContractCode + "解除担保，担保人：" + m.Guarantor.Name);
                });
            ...
            else if (creditGuarantyContractCount > 0 && credit.GuarantyContract.Count == model.GuarantyContract.Count)
            {
                mortgages.ToList().ForEach(m =>
                {
                    ...
                    {
                        // 担保物价值等发生变化
                        messageAppService.MessageTrack(id: mortgage.Id.Value, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.合同变更, name: "授信合同：" + credit.CreditContractCode + "解除担保，担保人：" + m.Guarantor.Name);
                    }
                });
            }
        }
        ...
        public void ChangeExpirationDate(CreditContract model)
        {
            ...
            messageAppService.MessageTrack(id: model.Id, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.合同变更, name: "授信合同：" + model.CreditContractCode + "有效日期变更");
        }
        ...
        public void StopStatus(Guid id)
        {
            ...
            // 报文追踪
            messageAppService.MessageTrack(id: credit.Id, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.终止合同, name: "授信合同：" + credit.CreditContractCode + "终止");
        }
        ...
        private void ChangeEffective(CreditContract model)
        {
            ...
            // 报文追踪(合同关键数据项金额发生变化)
            messageAppService.MessageTrack(id: model.Id, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.合同变更, name: "授信合同：" + model.CreditContractCode + "金额变更");
        }
    }
}
