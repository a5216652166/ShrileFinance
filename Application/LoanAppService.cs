namespace Application
{
    ...
    public class LoanAppService
    {
        ...
        public void ApplyLoan(LoanViewModel model)
        {
            ...
            // 报文追踪（放款）
            messageAppService.MessageTrack(referenceId: loan.Id, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.借款, name: "借据编号：" + loan.ContractNumber + "（放款）");
            messageAppService.MessageTrack(referenceId: loan.CreditId, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.借款, name: "贷款合同编号：" + creditRepository.Get(loan.CreditId).CreditContractCode + "（放款）");
        ...
        public void Payment(PaymentViewModel model)
        {
            ...
            foreach (var payment in payments)
            {
                ...
                && payment.ScheduledPaymentInterest == payment.ActualPaymentInterest)
                {
                    // 还款
                    // 报文追踪（还款信息记录）
                    messageAppService.MessageTrack(referenceId: payment.Id, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.还款, name: "借据：" + loan.ContractNumber + "还款");

                    // 报文追踪（五级分类调整 借据信息记录）
                    messageAppService.MessageTrack(referenceId: loan.Id, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.还款, name: "借据编号：" + loan.ContractNumber + "（放款）");
                }
                else if (payment.ScheduledPaymentPrincipal > payment.ActualPaymentPrincipal)
                {
                    // 逾期
                    // 报文追踪（五级分类调整 借据信息记录）
                    messageAppService.MessageTrack(referenceId: loan.Id, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.逾期, name: "借据：" + loan.ContractNumber + "逾期");
                }
                else
                {
                    // 欠息
                    // 报文追踪（五级分类调整 借据信息记录）
                    messageAppService.MessageTrack(referenceId: loan.Id, operationType: Core.Entities.CreditInvestigation.MessageOperationTypeEnum.欠息, name: "借据：" + loan.ContractNumber + "欠息");
                }
            }
        }
